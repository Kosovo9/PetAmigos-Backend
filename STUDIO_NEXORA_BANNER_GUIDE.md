# 🚀 LAUNCH BANNER - COPY/PASTE PARA STUDIO NEXORA

## 📋 PASO 1: Crear el componente LaunchBanner.tsx

**Ubicación:** `src/components/LaunchBanner.tsx`
...
export default function LaunchBanner() {
    ...
    React.useEffect(() => {
        ...
        if (LAUNCH_MESSAGES[browserLang as keyof typeof LAUNCH_MESSAGES]) {
            setLocale(browserLang);
        }
    }, []);

    const t = LAUNCH_MESSAGES[locale as keyof typeof LAUNCH_MESSAGES];
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Redirect a generación (AJUSTA LA RUTA SEGÚN TU APP)
        window.location.href = '/generate'; // O la ruta que uses
    };
...
## 🎯 PASO 4: Lógica de 3 fotos gratis

En tu componente de generación de fotos, agrega esta lógica:

```tsx
// Al inicio del componente
const launchUser = JSON.parse(localStorage.getItem('launch_user') || '{}');

// Antes de generar una foto, verifica:
if (launchUser.photosRemaining > 0 && launchUser.expiresAt > Date.now()) {
  // Usuario tiene fotos gratis disponibles
  generateFreePhoto();
  
  // Decrementar contador
  launchUser.photosRemaining--;
  localStorage.setItem('launch_user', JSON.stringify(launchUser));
} else {
  // Mostrar paywall o pricing
  showUpgradeModal();
}
```
...
- ✅ Auto-detecta idioma del navegador
- ✅ Guarda datos en localStorage
- ✅ Límite de 3 fotos por email
- ✅ 24 horas de acceso
