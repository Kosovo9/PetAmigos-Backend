...
import {
    DollarSign, Users, Heart, TrendingUp, Shield, Package,
    Eye, MousePointer, Download, Filter
} from 'lucide-react';
...
ers: 127,
    ...
    const tabs = [
        ...
        { id: 'advertisers', label: 'Anunciantes', icon: <Users className="h-4 w-4" /> },
        ...
        <ProtectedRoute allowedRoles={['admin']}>
            <div className="min-h-screen bg-gray-50">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <Card>
                                    ...
                                    <div className="space-y-3">
                                        {[
                                            { action: 'Nuevo anunciante registrado', user: 'Veterinaria Central', time: 'Hace 5 min' },
                                            { action: 'Donación procesada', user: 'Refugio Los Amigos', time: 'Hace 12 min' },
                                            { action: 'Campaña aprobada', user: 'Pet Store Max', time: 'Hace 23 min' },
                                        ].map((activity, i) => (
                                ...
                                <Card>
                                    <h3 className="text-lg font-bold mb-4">Métricas de Sistema</h3>
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between">
                                            <span className="text-gray-600">Usuarios activos hoy</span>
                                            <span className="font-bold">{metrics.dailyActive.toLocaleString()}</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-gray-600">Tasa de conversión</span>
                                            <span className="font-bold text-green-600">{metrics.conversion}%</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                        ...
                        <Card>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    ...
                                    <tbody>
                                        {[
                                            { name: 'Veterinaria Central', status: 'Activo', plan: 'Professional', spent: '$499' },
                                            { name: 'Pet Store Max', status: 'Activo', plan: 'Basic', spent: '$49' },
                                            { name: 'Clínica Veterinaria Norte', status: 'Pendiente', plan: 'Enterprise', spent: '$999' },
                                        ].map((adv, i) => (
                                            <tr key={i} className="border-b hover:bg-gray-50">
                                                <td className="py-3">{adv.name}</td>
                                                <td className="py-3">
                                                    <span className={`px-2 py-1 rounded-full text-xs ${adv.status === 'Activo' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                                                        }`}>
                                                        {adv.status}
                                                    </span>
                                                </td>
                    ...
                    {activeTab === 'shelters' && (
                        <Card>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {[
                                    { name: 'Refugio Los Amigos', animals: 245, received: '$3,450' },
                                    { name: 'Casa de Esperanza', animals: 189, received: '$2,890' },
                                    ...
                                    <div key={i} className="border rounded-lg p-4 hover:shadow-lg transition">
                                        ...
                                        <div className="space-y-2 text-sm">
                                            <div className="flex justify-between">
                                                <span className="text-gray-600">Animales ayudados:</span>
                                                <span className="font-semibold">{shelter.animals}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-600">Total recibido:</span>
                                                <span className="font-semibold text-green-600">{shelter.received}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                    ...
                    {activeTab === 'donations' && (
                        <Card>
                            <h3 className="text-xl font-bold mb-4">Historial de Donaciones</h3>
                            <div className="space-y-3">
                                {[
                                    { date: '2024-12-01', amount: '$456.90', refugios: 15, status: 'Distribuido' },
                                    { date: '2024-11-01', amount: '$423.50', refugios: 14, status: 'Distribuido' },
                                    { date: '2024-10-01', amount: '$476.50', refugios: 15, status: 'Distribuido' },
                                ].map((donation, i) => (
                                    <div key={i} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                                        <div>
                                            <p className="font-semibold">{donation.date}</p>
                                            <p className="text-sm text-gray-600">{donation.refugios} refugios beneficiados</p>
                                        </div>
